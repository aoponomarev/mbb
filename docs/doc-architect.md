...
- § Принципы кэширования — см. `docs/doc-cache.md` (отдельный документ с полным описанием стратегии и принципов кэширования)
- § Критически важные структуры
  ...
  - *AI провайдеры* — система провайдеров для работы с различными AI сервисами (YandexGPT, Perplexity AI), единый интерфейс, бесшовное переключение между провайдерами.
  ...
  - *Cloudflare интеграция* — интеграция с Cloudflare Workers, D1 и R2 для хранения данных пользователей, портфелей и датасетов. Google OAuth авторизация через Workers, синхронизация данных с облаком.
...
- `docs/doc-ai-providers.md` — общие принципы работы с AI провайдерами и механизм переключения между ними.
- `docs/doc-cache.md` — стратегия и принципы кэширования, правила принятия решений.
- `docs/doc-cloudflare.md` — инфраструктура Cloudflare, настройка Workers, D1, R2, Google OAuth.
- `docs/doc-cloudflare-integration-plan.md` — детальный план интеграции Google OAuth + Cloudflare с этапами и прогрессом.
- `docs/doc-cloudflare-testing.md` — тестовый план и чеклист для проверки интеграции.
...
- Дефолтная таймзона (`defaults.timezone`) вынесена из `app-footer.js` и компонентов настроек в `app-config.js`
- Дефолтный язык перевода (`defaults.translationLanguage`) вынесен из `app-footer.js` в `app-config.js`
- Дефолтные модели AI провайдеров (`defaults.perplexity.model`, `defaults.yandex.model`) и списки моделей вынесены в `app-config.js`
...
**ВЕРСИОНИРОВАТЬ, если:**
- ✅ Данные из внешних API (CoinGecko, AI провайдеры, и т.д.) — структура может измениться
...
- ❌ Настройки пользователя — пользователь не должен перенастраивать после каждого обновления (`theme`, `timezone`, `translation-language`, настройки AI провайдеров: API ключи, модели, выбор провайдера)
...
5. Модульная система загрузки:
   ...
   - Автоматическая загрузка всех модулей в правильном порядке:
     ...
     - Core модули (`core`): cache, validation, errors, api (market-metrics, ai-providers, ai-provider-manager), config (app-config, modals-config), events, state, logging (в порядке зависимостей)
     - Шаблоны (`templates`): button-template, dropdown-menu-item-template, dropdown-template, combobox-template, button-group-template, modal-template, modal-buttons-template, timezone-selector-template, ai-api-settings-template, app-header-template, app-footer-template
     - Библиотеки (`libraries`): vue
     - Компоненты (`components`): dropdown-menu-item, button, dropdown, combobox, button-group, modal, modal-buttons, timezone-selector, app-header, app-footer, modal-example-body, ai-api-settings, timezone-modal-body
...
См. отдельный документ `docs/doc-cache.md` с полным описанием стратегии и принципов кэширования, включая:
- Архитектуру системы кэширования
- Распределение данных по слоям хранения (hot/warm/cold)
- Версионирование приложения и критерии инвалидации кэша
- TTL и стратегии кэширования для каждого типа данных
- Правила принятия решений при добавлении новых ключей
