...
- **cache-only**: портфели пользователя, стратегии, настройки AI провайдеров (API ключи, модели, выбор провайдера), таймзона, язык перевода — только локальные данные, без TTL
...
Новости криптовалют, получаемые через AI провайдеры, не кэшируются для обеспечения актуальности. В кэше хранится только индекс текущей новости (0-4) и timestamp последнего запроса. При каждом клике на новость загружается свежая новость с переводом на выбранный язык. Это обеспечивает актуальность данных и экономит место в кэше, так как новости быстро устаревают. Кэширование учитывает текущий провайдер — данные от разных провайдеров кэшируются отдельно.
