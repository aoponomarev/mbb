...
- § *Иерархия уровней документирования* — структура документации по уровням специфичности и распределение контента.
  - *Разграничение между шапками шаблонов и скриптов компонентов* — принципы распределения информации между шапками шаблонов и скриптов Vue-компонентов.
...
- **Куда вносить изменения уровня 2:**
  ...
  - Документация компонентов приложения → `docs/doc-components.md` (навигация и краткие описания).
  - Принципы работы с компонентами → `docs/doc-comp-principles.md`.
  - Механизм управления CSS классами → `docs/doc-comp-classes.md`.
  - Практические аспекты реализации → `docs/doc-comp-implementation.md`.
...
**Разграничение между шапками шаблонов и скриптов компонентов:**
При документировании Vue-компонентов, разделенных на шаблон (`shared/templates/*-template.js`) и скрипт (`shared/components/*.js`), информация распределяется следующим образом:

**Шапка шаблона (`shared/templates/*-template.js`) содержит:**
- **Структуру HTML**: какие элементы, как вложены, какие теги используются.
- **Layout-решения**: padding, margin, flexbox/grid структура, выравнивание элементов.
- **CSS-классы для layout**: классы, используемые для позиционирования и отображения (например, `d-flex`, `align-items-center`, `p-0`).
- **Условный рендеринг**: что и когда показывается через `v-if`, `v-show` (структурная логика видимости).
- **Слоты и их область видимости**: какие слоты доступны, какие данные передаются в слоты.
- **Адаптивность на уровне layout**: какие элементы скрываются/показываются на разных брейкпоинтах через CSS-классы.
- **Технические детали шаблона**: почему шаблон вынесен (x-template), как он загружается.

**Шапка скрипта (`shared/components/*.js`) содержит:**
- **API компонента**: props, emits, slots (описание, не структура), методы ref API.
- **Логику компонента**: computed свойства, методы, watchers, обработка событий.
- **Интеграцию с внешними API**: Bootstrap JS API, инициализация, подписки на события, уничтожение экземпляров.
- **Состояние компонента**: data, реактивность, синхронизация состояния.
- **Бизнес-логику**: наследование стилей, маппинг данных, трансформации.
- **Особенности реализации**: стратегии совместимости, паттерны работы с DOM, обработка edge cases.

**Запрет дублирования:**
- Layout-информация (padding, margin, структура HTML) описывается **только** в шапке шаблона.
- В шапке скрипта при необходимости делается ссылка на шаблон: "Структура layout и CSS-классы см. в шапке шаблона `shared/templates/*-template.js`".
- Логика и API описываются **только** в шапке скрипта.
- В шапке шаблона при необходимости делается ссылка на скрипт: "Логика компонента и API см. в шапке компонента `shared/components/*.js`".
