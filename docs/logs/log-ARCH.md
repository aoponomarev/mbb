...
◆ Обновлены ссылки на переименованный файл документации в `core/lib-loader.js`
▶ Обновлена ссылка в комментарии `core/lib-loader.js`: `docs/doc-vue-lib.md` → `docs/doc-lib-vue.md`
◉ Синхронизировать ссылки в коде с переименованными файлами документации
@architect @refactoring @links @lib-loader

## Коммит от 22.12.2025 : Создание загрузчика библиотек с fallback-механизмом

◆ Создан `core/lib-loader.js` для загрузки внешних библиотек с автоматическим fallback при недоступности источников
▶ Создан `core/lib-loader.js` с поддержкой загрузки библиотек из нескольких источников: GitHub Pages CDN (`https://aoponomarev.github.io/libs/`) как основной источник, внешние CDN (jsdelivr, cdnjs) как fallback, локальное хранение как последний резерв. Реализованы функции: `load()` для загрузки одной библиотеки, `loadMultiple()` для загрузки нескольких, `isLoaded()` для проверки загрузки, `addSource()` для добавления источников. Кэширование загруженных библиотек для предотвращения повторной загрузки. Поддержка библиотек: vue, chartjs, numeral, vuedraggable, dayjs (альтернатива date-fns). Обновлён порядок загрузки в `docs/doc-architect.md`: добавлен lib-loader на шаг 12
◉ Обеспечить надёжную загрузку библиотек независимо от доступности отдельных CDN, централизовать управление библиотеками, подготовить инфраструктуру для использования Vue-библиотек
@architect @lib-loader @cdn @fallback @libraries
