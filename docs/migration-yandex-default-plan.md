...
> Оглавление `docs/migration-yandex-default-plan.md`
...
❌ **Настройки AI API:**
- Добавить поле Folder ID для YandexGPT в шаблон настроек
- Добавить поддержку Folder ID в компонент настроек
- Сохранять Folder ID в кэш

❌ **Проверка дефолтного провайдера:**
- Убедиться, что дефолтный провайдер установлен как 'yandex'
- Проверить, что настройки по умолчанию корректны

❌ **Тестирование:**
- Протестировать перевод tooltips через YandexGPT
- Протестировать получение новостей через YandexGPT
...
- [ ] Обновлен шаблон настроек (добавлено поле Folder ID)
- [ ] Обновлен компонент настроек (поддержка Folder ID)
- [ ] Добавлен ключ 'yandex-folder-id' в cache-config
- [ ] Протестированы настройки

## Этап 2: Дефолтный провайдер

- [ ] Проверен дефолтный провайдер в ai-provider-manager.js (= 'yandex')
- [ ] Проверен дефолтный провайдер в app-config.js (= 'yandex')
- [ ] Проверен дефолтный провайдер в компоненте настроек (= 'yandex')

## Этап 3: Перевод tooltips

- [ ] Проверено использование aiProviderManager в tooltips-translator.js
- [ ] Проверено кэширование с учетом провайдера
- [ ] Протестирован перевод tooltips через YandexGPT

## Этап 4: Новости в футере

- [ ] Проверено использование aiProviderManager в app-footer.js
- [ ] Проверен промпт для новостей
- [ ] Протестированы новости через YandexGPT

## Этап 5: Финальное тестирование

- [ ] Проведено комплексное тестирование всех функций
- [ ] Проверены логи (нет ошибок)
- [ ] Проверено переключение между провайдерами
