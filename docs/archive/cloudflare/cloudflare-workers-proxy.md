...
> **⚠️ УСТАРЕВШЕЕ РЕШЕНИЕ**: Этот документ описывает альтернативное решение через Cloudflare Workers, которое **НЕ используется** в текущей версии проекта. В настоящее время для проксирования запросов к YandexGPT API используется **Yandex Cloud Functions**. Актуальная документация находится в `docs/archive/yandex/`.
