...
- § Принципы работы с компонентами
  ...
  - *Работа с отступами в составных компонентах* — систематизация работы с gaps (отступами между дочерними элементами) в flexbox-контейнерах.
...
### Работа с отступами (gaps) в составных компонентах
...
### Порядок загрузки стилей компонентов
...
- `menuOffset` (Number | Array, default: null) — отступ между кнопкой и выпадающим меню через Popper.js offset. Число: `[0, offsetY]` (x, y в пикселях, только вертикальный отступ). Массив: `[offsetX, offsetY]` (x, y в пикселях). `null`: использовать дефолтный offset Bootstrap. Используется для уменьшения или увеличения расстояния между кнопкой и меню (например, `-8` для уменьшения отступа на 8px, что эквивалентно `-0.5rem` при стандартном размере шрифта).
...
- `verticalBreakpoint` (String) — адаптивная вертикальная ориентация: `'sm'` (576px). На мобильных (< 576px) группа отображается вертикально, на десктопе (>= 576px) — горизонтально. Если не указан, используется `vertical` для постоянной вертикальной ориентации.
