...
- § Принципы работы с компонентами
  ...
  - *Адаптивность текстов и надписей* — адаптивность текстового содержимого на мобильных устройствах.
- § Компонент dropdown
...
## Адаптивность текстов и надписей
Для компонентов с текстовым содержимым (кнопки, dropdown, элементы меню) применяется единый подход к адаптивности на мобильных устройствах:

1. **Приоритет иконки**: Если задана иконка, она заменяет текст на мобильных (текст скрывается через `d-none d-md-inline`). Иконка может отображаться только на мобильных (`d-inline d-md-none`).

2. **Короткие версии текста**: Если иконка не задана, но есть короткая версия текста (например, `labelShort` для кнопки, `buttonTextShort` для dropdown), она показывается на мобильных, а полная версия — на десктопе.

3. **Полные версии текста**: Показываются на десктопе или если нет коротких версий и иконок.

4. **Дополнительные элементы**: Вспомогательные элементы (например, `subtitle` в элементах меню) могут скрываться на мобильных через `d-none d-md-block`.

5. **Управление через `responsive` prop**: Объект `responsive` с флагами (`hideTextOnMobile`, `showIconOnMobile`, `useShortTextOnMobile`) позволяет гибко управлять адаптивным поведением.

**Применение**: Этот подход используется в компонентах `button`, `dropdown`, `dropdown-menu-item` и может применяться к другим компонентам с текстовым содержимым.
...
- `responsive` (Object, default: `{ hideTextOnMobile: true, showIconOnMobile: true, useShortTextOnMobile: true }`) — настройки адаптивности:
  - `hideTextOnMobile` (Boolean) — скрывать текст на мобильных, если есть иконка или укороченный текст.
  - `showIconOnMobile` (Boolean) — показывать иконку только на мобильных.
  - `useShortTextOnMobile` (Boolean) — использовать укороченный текст на мобильных, если нет иконки.
...
Компонент поддерживает адаптивное отображение кнопки триггера для мобильных устройств:
...
#### Дополнительные
- `menuClasses` (`String`, default: `''`) — дополнительные классы для dropdown-menu.
...
- Инициализация через `new bootstrap.Dropdown()` в `mounted()`.
- Подписка на события Bootstrap (`show.bs.dropdown`, `hide.bs.dropdown`).
- Программный доступ к Bootstrap API через методы компонента.
- Уничтожение экземпляра в `beforeUnmount()`.
...
- `responsive` (Object) — настройки адаптивности:
  ```javascript
  {
    hideTextOnMobile: true,  // скрывать текст на мобильных, если есть иконка или укороченный текст
