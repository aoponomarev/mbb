...
Этот подход используется в компонентах `button`, `dropdown`, `dropdown-menu-item` и может применяться к другим компонентам с текстовым содержимым. Вся логика адаптивности находится в `app/styles/components/`. Подробности реализации компонентов см. `docs/doc-comp-implementation.md` (раздел "Порядок загрузки стилей компонентов").
